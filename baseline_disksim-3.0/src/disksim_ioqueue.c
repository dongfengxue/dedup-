...
static void ioqueue_update_arrival_stats (ioqueue *queue, ioreq_event *curr)
{
 ...
 //  printf("line 565  ioqueue_update_arrival_stats       tdiff=%lf\n",tdiff);
   ...
   if (tmp->reqcnt == 1) {
     ...
     time_update(simtime-tmp->iob_un.time,is_print,2);                        //add by lhj,2表示读写的响应时间
      ...
      if (tmp->flags & READ) {
		 ...
		 time_update(simtime-tmp->iob_un.time,is_print,READ);                        //add by lhj,1表示读的响应时间
		 ...
		 time_update(simtime-tmp->iob_un.time,is_print,WRITE);                        //add by lhj,0表示写的响应时间
...
//   printf("ioqueue_add_new_request  queue->idlestart=%lf\n",queue->idlestart);
/*
...
*/
   queue->idlestart = simtime;
//	printf("ioqueue_physical_access_done   queue->idlestart=%lf\n",simtime);
...
void ioqueue_printstats (ioqueue **set, int setsize, char *sourcestr)    //ioqueue_printstats (&overallqueue, 1, prefix);
{
   int to_scheme = 0;
   int pri_scheme = 0;
   int numcomplete = 0;
