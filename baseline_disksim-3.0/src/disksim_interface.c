...
static void disksim_interface_io_done_notify (ioreq_event *curr)
{
  ...
  /* Added may remove Later */
#ifndef SYNTHIO 
  Request *req = (Request *) curr->buf;
  syssim_report_completion (MS_TO_SYSSIMTIME(simtime), req);
#else
  Request req;

  req.last_seek = disk_last_seektime;
  req.last_latency = disk_last_latency;
  req.last_cylno = disk_last_cylno;
  req.last_angle = disk_last_angle;
  req.blkno = curr->blkno;
  req.background = 0;
  req.devno = curr->devno;
  req.sectorcount = curr->bcount;
  req.start = curr->time;
  /* End may Remove Later*/
  syssim_report_completion (MS_TO_SYSSIMTIME(simtime), &req);
#endif
...
struct disksim * 
disksim_interface_initialize (void *addr, 
			      int len, 
			      const char *pfile, 
			      const char *ofile)
{
  
  return(disksim_interface_initialize_latency(addr, 
					      len, 
					      pfile, 
					      ofile, 
					      0,         // latency_weight
...
struct disksim * 
disksim_interface_initialize_latency (void *addr, 
				      int len, 
				      const char *pfile, 
				      const char *ofile, 
				      int latency_weight, 
...
void disksim_interface_shutdown (struct disksim *disksim_in, SysTime syssimtime)
{
   ...
   simtime = curtime;
   printf("disksim_interface_shutdown   simtime=%lf\n",simtime);
...
void disksim_interface_dump_stats (struct disksim *disksim_in, SysTime syssimtime)
{
   ...
   simtime = curtime;
   printf("disksim_interface_dump_stats  simtime=%lf\n",simtime);
   disksim_cleanstats();
   disksim_printstats();
