...
{
	//printf("io_catce_stray_events start!    curr->type=%d\n",curr->type);
   switch (curr->type) {
 ...
 //  printf("io_catch_stay_enents end!   curr->time=%lf,simtime=%lf,curr->bcount=%d\n",curr->time,simtime,curr->bcount);
}
...
//   printf("line 270 io_interrupt_arrive    intrp->type=%d\n",intrp->type);
...
//fprintf (outputfile, "%f: io_internal_event entered with event type %d, %f\n", curr->time, curr->type, simtime);
 //  printf ("io_internal_event  curr->time=%lf,io_internal_event entered with event type %d, %f\n", curr->time, curr->type, simtime);

   switch (curr->type) {
   case IO_REQUEST_ARRIVE:           //100
   ...
   case BUS_DELAY_COMPLETE:           //116

     bus_event_arrive(curr);
  // printf("bus_event end!\n");
     break;
...
event * io_get_next_external_event (FILE *iotracefile)
{
   ...
   temp = (ioreq_event *) getfromextraq();  //创建一个extrq内存池请求队列，如果是空的，创建，如果非空，队列指向下一个
   ...
   temp = iotrace_get_ioreq_event(iotracefile, disksim->traceformat, temp);   //读取trace，得到trance的的信息，tmp时间被赋值
  // printf("line   679     io_get_next_external_event: temp->time=%lf,simtime=%lf\n",temp->time,simtime);
   if (temp) {
      switch (disksim->traceformat) {
         case VALIDATE: io_validate_do_stats2 (temp);
		        break;
      }
      temp->type = IO_REQUEST_ARRIVE;          //此时的时间类型是请求到达
   //   printf("line 686  constintarrive=%lf,last_reuqest_arrive=%lf,temp->time=%lf \n",constintarrtime,last_request_arrive,temp->time);
      if (constintarrtime > 0.0) {
	 temp->time = last_request_arrive + constintarrtime;
	 last_request_arrive = temp->time;
      }
	//  printf("line 691  constintarrive=%lf,last_reuqest_arrive=%lf,temp->time=%lf \n",constintarrtime,last_request_arrive,temp->time);
	//  printf("line   692     io_get_next_external_event:  temp->time=%lf,simtime=%lf\n",temp->time,simtime);
      temp->time = (temp->time * ioscale) + tracebasetime;    //ioscale=1,tracebasetime=0;
   //  printf("line   691  io_get_next_external_event:  temp->time=%lf,simtime=%lf\n",temp->time,simtime);
      if ((temp->time < simtime) && (!disksim->closedios)) {    //时间顺序不对报错，temp->time是此条trace的time，simtime是上条trace的time
	...
	//  printf("tracemapping=%d\n",tracemappings);
      if (tracemappings) {
         io_map_trace_request(temp);   //不知道在干嘛
      }
      io_extq = (event *)temp;
      io_extq_type = temp->type;       //io_extq_typy 此时等于IO_request_arrive
    // printf("line   691     io_get_next_external_event:  temp->time=%lf,simtime=%lf,temp->type=%d\n",temp->time,simtime,temp->type);
   }
/*
