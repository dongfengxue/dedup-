...
#define HASH_LEN             257   //适应sha 256的长度 

#define TRACE_SCALE           3.8  //根据不同的trace放缩逻辑地址       

/* webmail 1-21   10G 1.9   5G
homes     1-10  10G   55
mail 1          10G   27.8
*/
/* 
...
typedef struct disksim {
   ...
   int    traceformat;         //trace 类型 ，dedup等
   int    endian;
   int    traceendian;    //trace存储方式，小端存储，大端存储
   int    traceheader;
   int    iotrace;			//运行的trace文件
   int    synthgen;          //是否同步
   int    external_control;
   void   (*external_io_done_notify)(ioreq_event *curr);
   FILE * parfile;           //打开的参数文件指针
   FILE * iotracefile;                 //运行trace文件指针
   FILE * statdeffile;
   FILE * outputfile;
   FILE * outios;
...
#define DM_MGR_SIZE 209715200     // number of sectors for 1gb
#define DEV_FLASH   1 
#define DEV_DISK    0 
...
//一维度表示读，写，sum，而维度表示时间戳
int time_dist[3][11];// 时间戳分布  0.02 0.04 0.08 0.10 0.12 0.14 0.16 0.20  》0.2  sum
 int is_print;
