...
_u32 pm_gc_cost_random()      //随机返回有无效页的blk，edit by lhj
{
 ...
 // printf("随机无效页最多的blk号=%d，随机选择的的无效页计数:%d\n",random_blk,nand_blk[random_blk].ipc);  //add by lhj
  return random_blk;
}
...
// printf("read last write %d\n",last_write);
 lpn = lsn/SECT_NUM_PER_PAGE;
  ...
  else  s_psn = pagemap[lpn].ppn * SECT_NUM_PER_PAGE;     //ÎïÀísecµÄ¸öÊý
 

  s_lsn = lpn * SECT_NUM_PER_PAGE;           //sec µÄ¸öÊý
...
//  victim_blk_no = pm_gc_cost_benefit();    //返回无效页最多的blk no
  victim_blk_no = pm_gc_cost_random();
  ...
  {
    valid_flag = nand_oob_read( SECTOR(victim_blk_no, i * SECT_NUM_PER_PAGE));   //¸ÃÒ³ÃæÊÇ·ñÓÐÐ§

    if(valid_flag == 1)  //¸ÃÒ³ÃæÓÐÐ§£¬Ç¨ÒÆ
    {
    	//printf("有效页迁移\n");
...
size_t pm_write(sect_t lsn, sect_t size, int mapdir_flag)     //ÏòÒ³ÃæÐ´¶«Î÷
{
  int i;
  int lpn = lsn/SECT_NUM_PER_PAGE;					
  int size_page = size/SECT_NUM_PER_PAGE;   	
  ...
  if (free_page_no[small] >= SECT_NUM_PER_BLK) 
  {
    if ((free_blk_no[small] = nand_get_free_blk(0)) == -1) 
    {
     ...
     if(free_blk_num<min_free_blk_num){    //edit by lhug//¿ÉÓÃblkÊýÁ¿Ð¡ÓÚ4£¬Æô¶¯GC£¬Ã¿×öÒ»´ÎÓÐÐ§blk+1
	     while (free_blk_num <min_free_blk_num+total_util_blk_num*0.1){   //每次gc  min_free_blk_num个块
	        j += pm_gc_run(small, mapdir_flag);
	      }
		 gc_count++;   //add by lhj
     }
	  printf("gc 次数=%d\n",gc_count);
      pm_gc_get_free_blk(small, mapdir_flag);             	//µÃµ½¿ÕÏÐblk£¬¸ù¾Ý²ÁÐ´´ÎÊýÕÒ
    } 
    else {
      free_page_no[small] = 0;
    }
...
int pm_init(blk_t blk_num, blk_t extra_num)
{
  ...
  pagemap_num = blk_num * PAGE_NUM_PER_BLK;     //pagemapµÄÒ³ºÅ

  pagemap = (struct pm_entry *) malloc(sizeof (struct pm_entry) * pagemap_num);  //pagemapµÄÖ¸Õë
  ...
  mapdir_num = (pagemap_num / MAP_ENTRIES_PER_PAGE);      //°üº¬¼¸¸ömapÄ¿Â¼
  ...
  free_blk_no[1] = nand_get_free_blk(0);   //¸ù¾Ý²Á³ý´ÎÊýÑ°ÕÒfree blk£¬1±íÊ¾Ð´µÄÊÇÓ³Éä±í£¬º¯ÊýÄÚ0±íÊ¾·ÇGCµ÷ÓÃ
  free_page_no[1] = 0;
