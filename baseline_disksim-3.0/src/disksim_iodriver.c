...
fprintf (outputfile, "done for real\n");
   ...
   devno = req->devno;
 //  printf("ioqueue_physicial_access_done  start                       !\n\n");
   req = ioqueue_physical_access_done(iodrivers[iodriverno]->devices[devno].queue, req);
 //  printf("ioqueue_physicial_access_done  end!\n");
   if (ctl) {
      ctl->numoutstanding--;
   }
   ...
   while (req) {
      ...
      if ((numreqs = logorg_mapcomplete(sysorgs, numsysorgs, tmp)) == COMPLETE) {
         ...
         //ioreq_event *
         temp = ioqueue_get_specific_request (overallqueue, tmp);
		 is_print=1;
       //  ioreq_event *
		 	temp2 = ioqueue_physical_access_done (overallqueue, temp);
		 is_print=0;
         ASSERT (temp2 != NULL);
...
*/
//printf("iodriver_respond_to_device  %f, Responding to device - cause = %d, blkno %d\n", simtime, req->cause, req->blkno);
...
//   printf("iodrive_get_time_to_handle_interrupt   retval=%lf,return=%lf\n",retval,curriodriver->scale * retval);
...
*/
   if ((iodrivers[iodriverno]->consttime == 0.0) || (iodrivers[iodriverno]->consttime == IODRIVER_TRACED_QUEUE_TIMES)) {
      ...
      iodriver_check_c700_based_status(iodrivers[iodriverno], infoptr->devno, infoptr->cause, IO_INTERRUPT_ARRIVE, infoptr->blkno);
   		//printf("intrp->time=%lf\n",intrp->time);
   } else {
      intrp->time = 0.0;
   }
  // printf("intrp->time=%lf\n",intrp->time);
   if (iodrivers[iodriverno]->type == STANDALONE) {     //都是走这个
      intrp->time += simtime;
      intrp->type = IO_INTERRUPT_COMPLETE;
      addtointq((event *) intrp);
	//  printf("iodrivers-type=standalone,intrp->time=%lf,simtime=%lf\n",intrp->time,simtime);
   } else {   //没走这个else
      tmp = getfromextraq();
      intrp->time /= (double) 5;
      tmp->time = intrp->time;
	 ...
	 // printf(" iodriver_interrupt_arrive  tmp->time=%lf\n",tmp->time);
...
*/
   ...
   ret = ioreq_copy (curr);
//	printf("ioqueue_add_new_request start!\n");
   ioqueue_add_new_request (overallqueue, ret);
 //  printf("iodriver request     line 728    overallqueue->time=%lf\n",overallqueue)
