...
#include "modules/disksim_ctlr_param.h"
...
//   printf("controller_bus_delay_complete start!\n");
/*
...
*/
   curr->next = NULL;
   curr->time = 0.0;
   if (busno == controller_get_upward_busno(currctlr, curr, &slotno)) {         //始终buswaitdir==up
      buswaitdir = UP;                                          //1
	//  printf("busno=%d,curctlr->inbusowned=%d,buswaitdir=%d\n",busno,currctlr->inbusowned,buswaitdir);
      if (busno == -1) {        //busno始终是-1
         intr_request ((event *)curr);
      } else {
         bus_deliver_event(busno, slotno, curr);
      }
      if (currctlr->inbusowned != -1) {       //inbusowned始终是-1
         bus_ownership_release(currctlr->inbusowned);
         currctlr->inbusowned = -1;
      }
...
controller *controller_copy(controller *orig) {
  ...
  result->cache = cache_copy(orig->cache);
