...
extern FILE *fp_gc;
double old_req_arr_time = 0.0;
double curr_req_arr_time = 0.0;
...
//kanishk begin
double curr_req_arr = 0.0;
double old_req_arr = 0.0;

int prev_blkno = 0;
int prev_bcount = 0;

double calculate_flash_resp(ioreq_event *curr)   //计算flash 的响应时间，没有调用
{
  double flash_resp;
  ...
  int req_size = curr->bcount; 

  int seq_flag;
  int curr_blkno = curr->blkno;
  if((prev_blkno + prev_bcount) == curr->blkno)
    seq_flag = 1;
  else seq_flag = 0;
  ...
  //tpch
  flash_resp = pow(2.718, flash_resp);


//  flash_resp = F_COEFF_INTARR_T * int_arr + F_COEFF_RW_T * rw_flag + F_COEFF_SEQ_T + F_COEFF_DS_PREV_T +  F_INTERCEPT;

  prev_blkno = curr->blkno;
  prev_bcount = curr->bcount;

  return flash_resp;
}

//hybrid
int io_redirect(logorg *currlogorg, ioreq_event *curr)
{
  int numreqs = 0;
  return numreqs;
}
...
static int logorg_maphybridstore (logorg *currlogorg, ioreq_event *curr)
{
   int numdisks;
   int stripeunit;
   int unitno;
...
*/
   ...
   //hybrid 
   } else if (maptype == HYBRIDSTORE) {
     numreqs = io_redirect(logorgs[logorgno], curr);
   //hybrid 
   } else {
...
static int logorg_distn(logorg *result, char *s) {
  ...
  //kanishk begin
  else if(!strcmp(s, "HybridStore")) { 
    //printf("Invoking hybrid store functionality\n");
    result->maptype = HYBRIDSTORE;
  } 
  //kanishk end
