...
void bus_delay(int busno, int devtype, int devno, double delay, ioreq_event *curr)
{
  ...
  // printf("bus_delay  tmp->time=%lf,simtime=%lf,delay=%lf\n",tmp->time,simtime,delay);
   addtointq((event *) tmp);
}
...
void bus_event_arrive(ioreq_event *ptr)
{
   ...
   if (curr->type == BUS_OWNERSHIP_GRANTED) {
	...
	//  printf("bus_event_arrive   simtime=%lf,curr->wait_start=%lf\n",simtime,curr->wait_start);
      stat_update (&currbus->arbwaitstats, (simtime - curr->wait_start));
   }
//printf("bus_event_arrive     curr->devtype=%d,simtime=%lf,cur->time=%lf,curr->type=%d\n",curr->devtype,simtime,curr->time,curr->type);
   switch (curr->devtype) {

      case CONTROLLER:              //5
         if (curr->type == BUS_DELAY_COMPLETE) {       //116
	    	controller_bus_delay_complete (curr->devno, curr->delayed_event, curr->busno);
		//	printf("after controller_bus_delay_complete    curr->devtype=%d,simtime=%lf,cur->time=%lf,curr->type=%d\n",curr->devtype,simtime,curr->time,curr->type);
         } else {
		// 	printf("controller_bus_ownership_grant\n");
...
*/
   devno = currbus->slots[slotno].devno;
//printf("bus_deliver_event    currbus->slots[slotno].devtype=%d\n",currbus->slots[slotno].devtype);
   switch (currbus->slots[slotno].devtype) {
