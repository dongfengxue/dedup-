...
void stat_print_set (statgen **statset, int statcnt, char *identstr)
{
   ...
   fprintf(outputfile,"runval(总共运行时间) =%f,runcount（总共运行条数）=%d，avg=runval\runcount \n",runval,runcount);    //add by lhj
   fprintf(outputfile, "%s%s average: \t%f\n", identstr, statdesc, avg);  //identstr:Overall I/O System   statdesc: response  time
