...
static void allocateextra ()   //	当extrq队列为空时，一次分配给extraq 128个event大小的内存段
{
   int i;
   event *temp = NULL;
...
//给intq队列添加事件，时间表示的是event何时发生的，intq是按照升序排列的
   ...
   case DISKSIM_SLAVE:
     {
       ...
       timediff = fabs(tmpevt.time - newint->time);   //得到浮点数的绝对值
...
static void initialize ()
{
   ...
   if (disksim->external_control | disksim->synthgen | disksim->iotrace) {
      io_initialize(val);             //模拟器相关的驱动器，device,bus,controller,iodriver等初始化
   }
   if (disksim->synthgen) {
...
{
   event *curr = getfromintq();      //从intq请求队列中取下一个事件
   event *temp;

   if (curr) {
      simtime = curr->time;
	//	printf("getnextevent     simetime=%lf,curr->type=%d,line 607\n",simtime,curr->type);
   
      if (curr->type == NULL_EVENT) {   //只在第一次走了一次
		if ((disksim->iotrace) && io_using_external_event(curr)) {   //io 外部请求类型
...
{
   event *curr;
	printf(" prime_simulation:simetime=%lf,line 690\n",simtime);
   ...
   if (disksim->iotrace) {                     //iotrace 读取输入
      ...
      if ((disksim->traceformat != VALIDATE) && (disksim->closedios == 0)) {
		 ...
		 //printf(" prime_simulation:curr->time=%d\n",curr->time);
         for (iocnt=1; iocnt < disksim->closedios; iocnt++) {
            ...
            if (curr->next) {
               ...
               curr->time = simtime + disksim->closedthinktime;
			//	printf(" prime_simulation:curr->time=%lf,line 715\n",curr->time);
               addtointq(curr->next);
            }
         }
      }
...
void disksim_simulate_event (int num)
{
  ...
  if ((curr = getnextevent()) == NULL) {       //读取tracefile,读取下一个队列
    disksim_simstop ();
  } 
  else {
...
//	      simtime, curr->type, num);   
  ...
  // printf(" disksim_simulate_event_748_line :curr->type=%d,curr->time=%lf\n",curr->type,curr->time);

    if (curr->type == INTR_EVENT) {          //98
      intr_acknowledge (curr);
    } 
    else if ((curr->type >= IO_MIN_EVENT) && (curr->type <= IO_MAX_EVENT)) {        //120=>x>=100
      io_internal_event ((ioreq_event *)curr);
    } 
    else if ((curr->type >= PF_MIN_EVENT) && (curr->type <= PF_MAX_EVENT)) {       // 1<x<97
      pf_internal_event(curr);
    }
    else if (curr->type == TIMER_EXPIRED) {            //x=121
      timer_event *timeout = (timer_event *) curr;
      (*timeout->func) (timeout);
    } 
    else if ((curr->type >= MEMS_MIN_EVENT)       //   200<=x<=220
	     && (curr->type <= MEMS_MAX_EVENT)) {
      io_internal_event ((ioreq_event *)curr);
    } 
    else if (curr->type == CHECKPOINT) {     //x=122
    ...
    else if (curr->type == STOP_SIM) {      //x=123
      disksim_simstop ();
    } 
    else if (curr->type == EXIT_DISKSIM) {        //x=124
      exit (0);
    } 
    else {
		printf("Unrecognized event in simulate: %d\n", curr->type);
...
void disksim_cleanup_and_printstats ()
{
	//尾延迟计算	 add bylhj
		...
		FILE *outputfile_time=fopen("base_time.txt","w");
		int i;
		float time=0.0;
		int sum=0;
		int sum_r=0;
		...
		if (outputfile_time== NULL) {
			fprintf(stderr, "Outfile2 cannot be opened for write access\n");
		exit(1);
		} 
	
		//总请求尾延迟分布
		fprintf(outputfile_time,"time \t	count\t    percentage\t \n");
		for(i=0;i<10;i++){
			time+=0.02;
			sum+=time_dist[2][i];
			fprintf(outputfile_time,"%f\t %d\t %f\t \n",time,time_dist[2][i],(float)sum/(float)time_dist[2][10]);
		}
		fprintf(outputfile_time,"sum=%d\n",time_dist[2][10]);
		
						//写尾延迟分布
		fprintf(outputfile_time,"\n write time \t	 count\t	read percentage\t \n");
		time=0.0;
		for(i=0;i<10;i++){
			time+=0.02;
			sum_w+=time_dist[0][i];
			fprintf(outputfile_time,"%f\t %d\t %f\t \n",time,time_dist[0][i],(float)sum_w/(float)time_dist[0][10]);
		}
		fprintf(outputfile_time,"write sum_w=%d\n",time_dist[0][10]);
	
							//读尾延迟分布
		fprintf(outputfile_time,"\n read time \t	count\t    read percentage\t \n");
		time=0.0;
		for(i=0;i<10;i++){
			time+=0.02;
			sum_r+=time_dist[1][i];
			fprintf(outputfile_time,"%f\t %d\t %f\t \n",time,time_dist[1][i],(float)sum_r/(float)time_dist[1][10]);
		}
		fprintf(outputfile_time,"read sum_r=%d\n",time_dist[1][10]);
