...
//printf("delay=%lf,read_delay=%lf,write_delay=%lf,erase_delay=%lf,gc_read_delay=%lf,gc_write_delay=%lf\n",delay,read_delay,write_delay,erase_delay,
	//	gc_read_delay,gc_write_delay);
...
void initFlash()
{
 ...
 // blk_t total_util_blk_num;
  ...
  total_util_sect_num  = flash_numblocks;       //可写数据的块个数
  total_extra_sect_num = flash_extrblocks;		//预留空间大小，每次gc的块数必须小于该值
 ...
 // min_free_blk_num=total_util_blk_num/8;      //设置的盘大小的1/8触发gc
  min_free_blk_num=total_util_blk_num*0.05;

 // if (nand_init(total_blk_num, 3) < 0) {   				//初始化闪存基本信息
  if (nand_init(total_blk_num,min_free_blk_num) < 0) {   				//初始化闪存基本信息
    EXIT(-4); 
  }
