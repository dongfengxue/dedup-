...
double simpleflash_get_blktranstime (ioreq_event *curr)
{
   ...
   //printf("simpleflash_get_blktranstime end!    tmptime=%lf\n",tmptime);
   return(tmptime);
}
 ...
 *获得总线（如果尚未获得），然后使用bus延时把事件发送到路径上。
*如果总线已经由该设备拥有或可以被获取立即（交叉总线）立即发送事件。
*否则，diskbusownershipgrant将会发送事件。
 */
...
static void simpleflash_bustransfer_complete (ioreq_event *curr)
{
   ...
   if (curr->flags & READ) {
    ...
    //  printf("simpleflash_bustransfer_complete     currtime->time=%lf,simtime=%lf,accesstime=%lf\n",curr->time,simtime,accessTime);
...
static void simpleflash_request_arrive (ioreq_event *curr)
{
  ...
  // printf("Kanishk: New request arrived! simpleflash_request_arrive!\n");
   ...
   /* create a new request, set it up for initial interrupt */  //创建一个新的请求，设置为初始中断
  ...
  //flashsim
  accessTime = simpleflash_get_acctime(curr->devno, curr->blkno, curr->bcount, (curr->flags & READ));
 // printf("accesstime=%lf\n",accessTime);
     
  /* initiate media access if request is a READ */
      if (curr->flags & READ) {   //read=1
         ioreq_event *tmp = ioreq_copy (curr);
         currflash->media_busy = TRUE;
         stat_update (&currflash->stat.acctimestats, accessTime);
...
static void simpleflash_completion_done (ioreq_event *curr)
{
   ...
   if (curr != NULL) {
      ...
      //printf ("Kanishk access time call 2.5\n");
         curr->time = simtime + accessTime;  //总时间延迟
...
void simpleflash_event_arrive (ioreq_event *curr)
{
   ...
   // fprintf (outputfile, " - devno %d, blkno %d, type %d, cause %d, read = %d\n", curr->devno, curr->blkno, curr->type, curr->cause, curr->flags & READ);
	//printf("simpleflash_event_arrive      curr->type=%d,curr->devno=%d,curr->time=%lf\n",curr->type,curr->devno,curr->time);
   currflash = getsimpleflash (curr->devno);
...
double  simpleflash_get_acctime (int devno, unsigned int blkno, int bcount, int flags)
{
  double delay = 0;
  double temp; 
  
  delay = callFsim(blkno, bcount, flags);    //得到的是总写入，擦除，读取的延迟
  DEVICE_SERVICE_TIME = delay;          //设备的服务时间
...
struct simpleflash *disksim_simpleflash_loadparams(struct lp_block *b)  //初始化执行了一次
{
  ...
  //printf("disksim_simpleflash_loadparams;\n");
