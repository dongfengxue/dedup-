...
//flashsim: end
//static double hashtime=0.02;          //edit by lhj 假设hash计算时间
...
static void simpleflash_request_arrive (ioreq_event *curr)
{
  ...
  //flashsim
  accessTime = simpleflash_get_acctime(curr->hash,curr->devno, curr->blkno, curr->bcount, (curr->flags & READ));
  ...
  /* initiate media access if request is a READ */
      ...
      //printf ("Kanishk access time call 1.5\n");
         tmp->time = simtime + accessTime;    //原版
      //  tmp->time =simtime +accessTime +hashtime;   //edit by lhj
