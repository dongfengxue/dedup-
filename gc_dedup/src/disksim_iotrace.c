...
static ioreq_event * iotrace_dedup_get_ioreq_event (FILE *tracefile, ioreq_event *new)
{
   ...
   char line[401];
   static int t_c=0;
   static int laddr_c=0;    //逻辑地址的偏移
    struct lpn_tree_node *lpn_tmp = NULL;
	int blkno;
	int trace_scale;
   laddr_c++; 
   t_c++;
  ...
  // if (fgets(line, 200, tracefile) == NULL) {
  	if(fgets(line,400,tracefile)==NULL){   //适应sha256长度的偏移
      addtoextraq((event *) new);
      return(NULL);
   }
   
   if (sscanf(line, "%lf %d %d %d %s\n", &new->time,  &new->blkno, &new->bcount, &new->flags,new->hash) != 5) {
   //	if (sscanf(line, "%lf %d %d %d %s\n", &new->time,  &blkno, &new->bcount, &new->flags,new->hash) != 5) {
      fprintf(stderr, "Wrong number of arguments for I/O trace event type\n");
      fprintf(stderr, "line: %s", line);
      ddbg_assert(0);
   }
//	 if(new->blkno >lhj_max_laddr){
//		lhj_max_laddr=new->blkno;
//   }

   new->devno=0;
	// trace_scale=20;
	 new->blkno/=TRACE_SCALE;
  // new->time+=t_c;
//  new->time/=100000;         //100000,让时间更紧凑，请求更加密集
 // new->time/=100;
   //flashsim
   new->bcount = ((new->blkno+ new->bcount-1)/4 - (new->blkno)/4 + 1) * 4;
  // new->blkno=new->blkno+4*laddr_c;      //add by lhj
 // new->blkno=new->blkno;
   new->blkno /= 4; 
   new->blkno *= 4;
   new->blkno %= (max_laddr-4096);
//	new->blkno%=max_laddr;
  new->blkno+=4000;
 ...
 //************************************************lpn 映射************************//
 /*
  lpn_tmp = iterative_lpn_rbtree_search_lpn(lpn_root, blkno);
  if(lpn_tmp==NULL){
	   
	   struct ftl_l_p *lpn_node = NULL;
	   
	   my_curr_lpn+=4; 
		printf("NULL blkno is %d my_curr_lpn %d\n",blkno,my_curr_lpn);
	   lpn_node = create_ftl_l_p_node(my_curr_lpn,blkno);
	   insert_lpn_rbtree_lpn(lpn_root,lpn_node);
	   new->blkno = my_curr_lpn;
  }else{
	   printf("not NULL blkno is %d\n",blkno);
	   new->blkno = lpn_tmp->key->curr_laddr;
 	 }
  assert(my_curr_lpn < max_laddr);
   if(my_curr_lpn > max_laddr){
	   printf("no free space yet\n");
	  
	   exit(0);
   }

   new->flags=0;                               //先设定flag为0，全部是写操作
    new->blkno /= 4; 
   new->blkno *= 4;
    new->bcount = ((new->blkno+ new->bcount-1)/4 - (new->blkno)/4 + 1) * 4;
*/
