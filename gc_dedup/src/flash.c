...
void nand_stat_print(FILE *outFP)
{
  ...
  fprintf(outFP,"lhj add flash read(#):%8u  ",flash_read_num);       //add by lhj
  fprintf(outFP,"lhj add flash write(#):%8u  ",flash_write_num);       //add by lhj
  fprintf(outFP,"lhj add flash erase(#):%8u \n ",flash_erase_num);       	//add by lhj
  ...
  fprintf(outFP, "------------------------------------------------------------\n");
  fprintf(outFP,"lhj GC page write:%8u  ",gc_page_write);
  fprintf(outFP,"lhj Block erase:%8u\n",blk_erase);
  
  fprintf(outFP,"flash erase num %8u\n ",flash_erase_num);
  fprintf(outFP,"flash gc write num %8u\n",flash_gc_write_num);
}
...
int nand_init (_u32 blk_num, _u8 min_free_blk_num)
{
  ...
  nand_blk_num = blk_num;    //total blk num

  pb_size = 1;
  min_fb_num = min_free_blk_num;       //最少可用块数量,lhj自己设置的常量MIN_FREE_BLK_NUM
  ...
  free_blk_num = nand_blk_num;     //total blk num
...
/**************** NAND OOB READ **********************/
int nand_oob_read(_u32 psn)   //读该页面是否有效
{
...
_u8 nand_page_read(_u32 psn, _u32 *lsns, _u8 isGC)
{
  ...
  if(nand_blk[pbn].state.free != 0) {
    for( i =0 ; i < 8448 ; i++){
   //  for(j =0; j < 256;j++){
    	for(j=0;j<SECT_NUM_PER_BLK;j++){      //edit by lhj
  ...
  if (isGC == 1) {
    for (i = 0; i < SECT_NUM_PER_PAGE; i++) {
      ...
      if((nand_blk[pbn].sect[pin + i].free == 0) &&
        ...
        lsns[valid_sect_num] = nand_blk[pbn].sect[pin + i].lsn;//----------获取逻辑地址
        valid_sect_num++;
      }
    }
    ...
    for (i = 0; i < SECT_NUM_PER_PAGE; i++) {
      if (lsns[i] != -1) {
        ...
        valid_sect_num++;
        //----------------------------读空块处理------------------------------//
        //if(nand_blk[pbn].sect[pin + i].free != 0 || nand_blk[pbn].sect[pin + i].valid == 1 || nand_blk[pbn].sect[pin + i].lsn != lsns[i])
			//valid_sect_num++;
        //----------------------------读空块处理--------xia----------------------//
     //   valid_sect_num++;
      }
...
_u8 nand_page_write(_u32 psn, _u32 *lsns, _u8 isGC, int map_flag)
{
  ...
  if(pbn >= nand_blk_num){
    printf("pbn=%d,nand_blk_num=%d,break !\n",pbn,nand_blk_num);
  }
  ...
  for (i = 0; i <SECT_NUM_PER_PAGE; i++) {
    ...
    if (lsns[i] != -1) {
      ...
      if(nand_blk[pbn].state.free == 1) {
        printf("nand_blk[pbn].state.free==1,pbn = %d\n",pbn);
      }

		if(nand_blk[pbn].sect[pin+i].free!=1)	//add by lhj
			printf("nand_blk[pbn].sect[pin+i].free!=1,pbn=%d,pin=%d,i=%d",pbn,pin,i);
  ...
  nand_blk[pbn].blk_ref++;    //add by lhj
...
/********************lhg****set_nand_blk_lpn***********/
void set_nand_blk_lpn(_u32 psn, _u32 lsn)
{
  ...
  //  int i;
   /*  for (i = 0; i < SECT_NUM_PER_PAGE; i++) 
	{
    nand_blk[pbn].sect[pin + i].lsn == lsn+i;
     }
     */
    nand_blk[pbn].sect[pin].lsn=lsn;
  ASSERT(nand_blk[pbn].state.free == 0);    //add by lhj
  ASSERT(nand_blk[pbn].sect[pin].free == 0);
  ASSERT(nand_blk[pbn].sect[pin].valid=1);
  
 // printf("pbn=%d,pin=%d,lsn=%d\n",pbn,pin,lsn);
}

/************xia********lhg****set_nand_blk_lpn***********/
...
void nand_erase (_u32 blk_no)
{
  ...
  blk_erase++;  //add by lhj
}

/**************** NAND INVALIDATE **********************/
void nand_invalidate (_u32 psn, _u32 lsn)
{
  _u32 pbn = BLK_F_SECT(psn);  //block num
  _u16 pin = IND_F_SECT(psn);  //sect in block
...
_u32 nand_get_free_blk (int isGC)      //得到的是擦除次数最少的空闲blk，返回-1说明没有free blk
{
  _u32 blk_no = -1, i;
  int flag = 0,flag1=0;
  flag = 0;
  ...
  if ((isGC == 0) && (free_blk_num<=min_free_blk_num)) {   //free_blk_num<min_free_blk_num,说明需要gc
    printf("触发最少的空闲blk数目: %d\n", min_free_blk_num);
    return -1;
  }

  for(i = 0; i < nand_blk_num; i++) 
  {
    if (nand_blk[i].state.free == 1) {       //该可用块可用
      flag1 = 1;                          //表示空闲

      if ( nand_blk[i].state.ec < MIN_ERASE ) {
	  	//	if(nand_blk[i].fpc==SECT_NUM_PER_BLK){          //add by lhj
            blk_no = i;
            MIN_ERASE = nand_blk[i].state.ec;
            flag = 1;                   //flag=1表示找到可用的擦除次数最少的blk
	  	//	}
      }
    }
  }
  ...
  if ( flag == 1) {
		...
		if(nand_blk[blk_no].fpc != SECT_NUM_PER_BLK)
		{	
		printf("blk_no=%d,fpc=%d\n",nand_blk[blk_no].fpc);
	//	nand_blk[blk_no].fpc=SECT_NUM_PER_BLK;
	//	nand_blk[blk_no].ipc=0;
	//	nand_blk[blk_no].lwn=-1;
		}
...
_u32 nand_get_free_blk_high_ref(int isGC)      //得到的是擦除次数最少的空闲blk，返回-1说明没有free blk
{
  _u32 blk_no = -1, i;
  int flag = 0,flag1=0;
  flag = 0;
