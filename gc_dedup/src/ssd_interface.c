...
double calculate_delay_flash()
{
  ...
  hash_delay = (double)HASH_DELAY *compute_hash_num;

  delay = read_delay + write_delay + erase_delay + gc_read_delay + gc_write_delay + 
    oob_read_delay + oob_write_delay + hash_delay;
...
//min_free_blk_num=total_util_blk_num/8;              //add by lhj,剩余多少触发gc
 min_free_blk_num=total_util_blk_num*0.05;                         //20%触发gc
