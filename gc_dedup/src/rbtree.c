...
static void inorder(RBTree tree)
{
	...
	//	printf("  ref:%d ,hash:%s\n",tree->key->ref,tree->key->md);
		...
		if(tree->key->ref==1)
			ref_1_num++;
		else if(tree->key->ref ==2)
			ref_2_num++;
		else if(tree->key->ref==3)
			ref_3_num++;
		else if(tree->key->ref==4)
			ref_4_num++;
		else if(tree->key->ref==5)
			ref_5_num++;
		else
			ref_high_num++;
        inorder(tree->right);
...
void delete_rbtree_paddr(RBRoot *root, int key)
{
	...
	//	printf("delete addr:addr=%d\n",key);
        rbtree_delete(root, z);
    }
	else
		printf("dele addr error!");
}
...
int  delete_laddr_in_data_node(struct data_node *data_tmp,int laddr)
{
...
void insert_laddr_in_node(struct data_node *data_tmp,int laddr)
{
	...
	laddr_l_tmp = data_tmp->laddr_l;
	/*#if DEBUG_PRINTF
	printf("ref %d laddr is : ",data_tmp->ref);
	#endif
