...
//   hash_delay = (double)HASH_DELAY *flash_write_num;       //add by lhj
// hash_delay = (double)HASH_DELAY ;       //add by lhj
 
 /* delay = read_delay + write_delay + erase_delay + gc_read_delay + gc_write_delay + 
    oob_read_delay + oob_write_delay;      //原版
    */
    delay = read_delay + write_delay + erase_delay + gc_read_delay + gc_write_delay + 
    oob_read_delay + oob_write_delay;             //add by lhj

	//printf("flash_read_num=%d,flash_write_num=%d\n",flash_read_num,flash_write_num);
  ...
  if(warm_done == 1){
    fprintf(fp_gc_timeseries, "lhj :%d\t%d\t%d\t%d\t%d\t%d\n", 
...
void initFlash()
{
 ...
 //  fprintf (outputfile, "total_blk_num: %d ,total_util_blk_num: %d \n", total_blk_num, total_util_blk_num);
...
min_free_blk_num=total_blk_num/16;              //add by lhj,剩余多少触发gc
 fprintf (outputfile, "total_blk_num: %d ,total_util_blk_num: %d,min_free_blk_num:%d \n", total_blk_num, total_util_blk_num,min_free_blk_num);

if(nand_init(total_blk_num, min_free_blk_num) < 0) {    //edit by lhj
    EXIT(-4); 
  }
...
void send_flash_request(char hash[],int start_blk_no, int block_cnt, int operation, int mapdir_flag)
{
	int size;
	static int offset=0;
	//size_t (*op_func)(sect_t lsn, size_t size);
	size_t (*op_func)(char hash[],sect_t lsn, size_t size, int mapdir_flag);

	//	ASSERT(start_blk_no+block_cnt<total_util_sect_num)
        if((start_blk_no + block_cnt) >= total_util_sect_num){
          printf("start_blk_no+block_cnt>=totol_util_sect_num,start_blk_no: %d, block_cnt: %d, total_util_sect_num: %d\n", 
              start_blk_no, block_cnt, total_util_sect_num);
          exit(0);
        }
		
		/*
	    start_blk_no=start_blk_no+offset;    //add by lhj
		offset++;
		start_blk_no/=4;
		start_blk_no*=4;
	     start_blk_no %= (max_laddr-4096);
  	    start_blk_no+=4000;                   //add by lhj
  	    */
/*
		ASSERT(start_blk_no+block_cnt<total_util_sect_num)
        if((start_blk_no + block_cnt) >= total_util_sect_num){
  ...
  */	    
	...
	case 0:
		...
		while (block_cnt>0) {
			size = op_func(hash,start_blk_no, block_cnt, mapdir_flag);
			disk_write++;             //每做一次写操作，磁盘写入次数++，每次写入2K
			start_blk_no += size;
			block_cnt-=size;
		//	printf("start_blk_no=%d,block_cnt=%d,total_util_sect_num=%d\n",start_blk_no,block_cnt,total_util_sect_num); //add by lhj
		}
		break;
	//read
	case 1:
...
double callFsim(char hash[],unsigned int secno, int scount, int operation)
{
  double delay; 
  int bcount;
...
*/
  ...
  switch(operation)
  {
    ...
    while(cnt > 0)
    {
        ...
        // page based FTL
        if(ftl_type == 1){
		//  printf("send_flash_request start\n");
          send_flash_request(hash,blkno*SECT_NUM_PER_PAGE, SECT_NUM_PER_PAGE, operation, 1);  
		//发送flash请求，每次请求一个页大小
	//	 printf("send_flash_request end\n");
          blkno++;
        }

        // blck based FTL
       /* else if(ftl_type == 2){
          send_flash_request(hash,blkno*4, 4, operation, 1); 
          blkno++;
        }
...
//  if(return_MD5time)
//   delay = calculate_delay_flash()+return_MD5time;
//  else
