...
static void simpleflash_request_arrive (ioreq_event *curr)
{
  ...
  //flashsim
  accessTime = simpleflash_get_acctime(curr->hash,curr->devno, curr->blkno, curr->bcount, (curr->flags & READ)); //edit by lhj,加上hash时间
 // accessTime = hashtime+simpleflash_get_acctime(curr->hash,curr->devno, curr->blkno, curr->bcount, (curr->flags & READ));
  /* initiate media access if request is a READ */
	...
	//  tmp->time =simtime +accessTime ;   //edit by lhj,加上hash时间
...
double  simpleflash_get_acctime (char hash[], int devno, unsigned int blkno, int bcount, int flags)
{
  double delay = 0;
  double temp; 
  
  delay = callFsim(hash,blkno, bcount, flags); 
