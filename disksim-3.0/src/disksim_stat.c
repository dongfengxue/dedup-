...
void time_update (double value,int flag,int type)   //统计响应时间分布 value是时间，flag表示是否打印统计一次
{//type  2  read/write   1 write   0 read   
...
static void stat_print_large_dist (statgen **statset, int statcnt, int count, char *identstr)
{
   ...
   for (i=0; i<buckets; i++) {
      ...
      density = (double) bucketcnt / (double) count;   //当前条数所占百分比
      dist += density;           //累积百分比
      if (scaled) {
...
void stat_print_file (statgen *statptr, char *identstr, FILE *outfile){
   ...
   for (i=(DISTSIZE-buckets); i<(DISTSIZE-1); i++) {           //输出尾延迟分布
      if (i >= statptr->equals) {
         distchar = '<';
      }
      if (statptr->scale == 1) {
...
void stat_print_set (statgen **statset, int statcnt, char *identstr)
{
  ...
  // fprintf(outputfile,"runval =%f,runcount=%d\n",runval,runcount);    //add by lhj
   fprintf(outputfile, "%s%s average: \t%f\n", identstr, statdesc, avg);        //平均响应时间
   ...
   if (buckets > DISTSIZE) {
   	  fprintf(outputfile,"stat_print_large_dist\n");
      stat_print_large_dist(statset, statcnt, runcount, identstr);   //打印输出时间分布具体
      return;
   }
   fprintf(outputfile, "%s%s distribution\n", identstr, statdesc);
   statptr = statset[0];
