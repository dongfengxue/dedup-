...
_u32 opm_gc_cost_benefit()
{
  ...
  for (i = 0; i < nand_blk_num; i++) {
    //if(i == free_blk_no[0] || i == free_blk_no[1]){   //the disksim3.0
	if(i == free_blk_no[0] || i == free_blk_no[1]||i==free_blk_no[2]){ //add by lhj
      continue;
    }
...
size_t opm_read(char hash[],sect_t lsn, sect_t size, int mapdir_flag)
{
  int i;
...
size_t opm_write(char hash[],sect_t lsn, sect_t size, int mapdir_flag)  
{
  int i;
  int lpn = lsn/SECT_NUM_PER_PAGE; // logical page number
  int size_page = size/SECT_NUM_PER_PAGE; // size in page 
...
int opm_init(blk_t blk_num, blk_t extra_num)
{
  ...
  for(i = 0; i<mapdir_num; i++){
    ...
    opm_write("sb",i*SECT_NUM_PER_PAGE, SECT_NUM_PER_PAGE, 2);
  }

  /*
  for(i = mapdir_num; i<(opagemap_num - mapdir_num - (extra_num * PAGE_NUM_PER_BLK)); i++){
    opm_write("sb",i*SECT_NUM_PER_PAGE, SECT_NUM_PER_PAGE, 1);
  }
  */
