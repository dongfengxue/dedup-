...
*/
   ...
   if (tmp->reqcnt == 1) {
	 ...
	 queue->readlen--;
	  ...
	  time_update(simtime-tmp->iob_un.time,is_print,2);                        //add by lhj,2表示读写的响应时间
      ...
      if (tmp->flags & READ) {
		 ...
		 time_update(simtime-tmp->iob_un.time,is_print,READ);                            //add by lhj
      } else {
         if (tmp->flags & TIME_CRITICAL) {
		 ...
		 time_update(simtime-tmp->iob_un.time,is_print,WRITE); 						//add by lhj
      }
   } else {
      trv = tmp->iolist;
      queue->iobufcnt--;
...
static void ioqueue_printqueuestats (ioqueue **set, int setsize, char *prefix)
{
   ...
   fprintf (outputfile, "%s lhj  only once:    %d\n", prefix, maxwritelen);
   stat_print_set(statset, (3*setsize), prefix);
   free(statset);
}
...
static void ioqueue_subqueue_printstats (subqueue **set, int setsize, char *prefix, int printcritstats)
{
   ...
   for (i=0; i<setsize; i++) {
      statset[i] = &set[i]->outtimestats;    //响应时间
   }
   stat_print_set(statset, setsize, prefix);
   ...
   for (i=0; i<setsize; i++) {
      statset[i] = &set[i]->qtimestats;   //queue time
   }
   stat_print_set(statset, setsize, prefix);

   for (i=0; i<setsize; i++) {
      statset[i] = &set[i]->accstats;   //physical access time
   }
   stat_print_set(statset, setsize, prefix);
   free(statset);
}
...
void ioqueue_printstats (ioqueue **set, int setsize, char *sourcestr)
{
   ...
   for (i=0; i<setsize; i++) {
	 ...
	 pri_scheme = set[i]->pri_scheme;
      ...
      statset[(3*i)] = &set[i]->base.outtimestats;    //响应时间
...
*/
   ...
   stat_print_set(statset, (3*setsize), sourcestr);
   	fprintf(outputfile,"setsize is \t %d add by lhj\n",setsize);   //add by lhj
   ...
   ioqueue_printqueuestats(set, setsize, sourcestr);   //sourcestr=overall io system
