...
{
  int c;
  struct lp_block *result = malloc(sizeof(struct lp_block));
  memcpy(result, b, sizeof(struct lp_block));
  result->params = malloc(b->params_len * sizeof(int *));
  bzero(result->params, b->params_len * sizeof(int *));
...
static struct lp_list *copy_list(struct lp_list *l) {
  int c;
  struct lp_list *result = malloc(sizeof(struct lp_list));
  memcpy(result, l, sizeof(struct lp_list));
  result->values = malloc(l->values_len * sizeof(int *));
...
static struct lp_value *copy_value(struct lp_value *v) {
  struct lp_value *result = malloc(sizeof(struct lp_value));
...
static struct lp_param *copy_param(struct lp_param *p) {
  struct lp_param *result = malloc(sizeof(struct lp_param));
...
static void indent(FILE *f) {
  char *space = malloc(3*indent_level+1);
...
{
  struct lp_tlt *result = malloc(sizeof(struct lp_tlt));
...
int lp_add_type(char *newt, char *parent) {
  int c;
  struct lp_subtype *st = malloc(sizeof(struct lp_subtype));
  ...
  if(!lp_lookup_type(newt,0)) {
    ...
    lp_typetbl = realloc(lp_typetbl, 
			 2 * lp_typetbl_len * sizeof(int *));
...
void lp_init_typetbl(void) {
  int c;
  lp_typetbl = malloc(lp_max_mod * sizeof(struct subtype *));
  bzero(lp_typetbl, lp_max_mod * sizeof(struct subtype *));
